...
namespace MessagePackCompiler.Generator
{
    public static class EmbedStringHelper
    {
        ...
        public static void ToByteArrayString(StringBuilder builder, byte[] binary)
        {
            var headerLength = GetHeaderLength(binary.Length);
            Span<byte> header = stackalloc byte[headerLength];
            EmbedHeader(binary.Length, header);
            builder.Append("new byte[").Append(headerLength).Append(" + ").Append(binary.Length).Append("] { ").Append(header[0]);
