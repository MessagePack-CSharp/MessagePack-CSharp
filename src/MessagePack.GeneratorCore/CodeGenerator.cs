...
namespace MessagePackCompiler
{
    public class CodeGenerator
    {
        ...
        /// <param name="checkInputName">dash customizing</param>
           ...
           bool checkInputName)
        {
            ...
            foreach (var multioutSymbol in multipleOutputSymbols.Length == 0 ? new[] { string.Empty } : multipleOutputSymbols)
            {
                ...
                logger($"CheckInputMode : {checkInputName}");

                var collector = new TypeCollector(namespaceDot, checkInputName, compilation, true, useMapMode, externalIgnoreTypeNames, x => Console.WriteLine(x));
                ...
                if (Path.GetExtension(output) == ".cs")
                {
                    ...
                    sb.AppendLine("    {");
                    sb.AppendLine("        public static void Declare(global::MessagePack.MessagePackSerializerOptions options)");
                    sb.AppendLine("        {");
                    foreach (IResolverRegisterInfo info in resolverTemplate.RegisterInfos)
                    {
                        sb.AppendLine($"            options.Resolver.GetFormatterWithVerify<{info.FullName}>();");
                    }
