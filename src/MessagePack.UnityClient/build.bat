...
echo on
start "Unity PACKAGE" /wait "c:\Program Files\unity\editor\Unity.exe" -batchmode -quit -nographics -buildTarget standalone -projectPath "%~dp0\" -executeMethod PackageExport.Export -logfile "%LogDirectory%\unity_package.log"
@IF ERRORLEVEL 1 (
