...
{
    internal static void Serialize(ref MessagePackWriter writer, ref readonly int input, int length)
    {
        ...
        const int maxInputSize = int.MaxValue / (sizeof(int) + 1);
        if (Vector128.IsHardwareAccelerated)
        {
            ...
            for (var alignedInputLength = (nuint)(length & mask); alignedInputLength > 0; alignedInputLength = (nuint)(length & mask))
            {
                ...
                var destination = writer.GetSpan((int)alignedInputLength * (sizeof(int) + 1));
