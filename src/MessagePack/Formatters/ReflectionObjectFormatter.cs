...
public sealed partial class ReflectionObjectFormatter<TWriteBuffer, TReadBuffer, T> : IMessagePackFormatter<TWriteBuffer, TReadBuffer, T>
{
    ...
    readonly record struct MemberCandidate(MemberInfo Member, Type MemberType, bool IsPublic, string KeyName);
    ...
    void BuildContractlessTable(List<MemberCandidate> candidates)
    {
        ...
        for (int i = 0; i < included.Count; i++)
        {
            ...
            keys[i] = Encoding.UTF8.GetBytes(included[i].KeyName); // shadowed members carry their qualified key
        ...
        foreach (var candidate in candidates)
        {
            ...
            if (intKey is null && stringKey is null)
            {
                if (keyAsPropertyName)
                {
                    ...
                    stringKey = candidate.KeyName != name ? candidate.KeyName : KeyNamingPolicyConverter.ConvertName(namingPolicy, name);
                }
    ...
    // Parameter binding (v3's DynamicObjectResolver rule, issue #1016): a parameter binds the member whose string key
    // matches its name, and only when no key matches, the member whose declared name matches. keyNames is null for
    // int-keyed and contractless tables, where only the name pass applies. The source generator's
    // TryMatchConstructor implements the same rule.
    ...
    void SelectConstructor(string[] memberNames, string?[]? keyNames)
    {
        ...
        var attributedConstructors = typeof(T).GetConstructors(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance)
            .Where(static c => FindAttribute(c.GetCustomAttributes(inherit: false), MessagePackAttributeNames.SerializationConstructor) is not null)
            .ToArray();
        if (attributedConstructors.Length > 1)
        {
            // the generator's MsgPack009 for the same shape: a first-in-declaration-order pick would tie the contract
            // to source layout (v3 crashed on SingleOrDefault here)
            throw new MessagePackSerializationException($"'{typeof(T).FullName}' declares [SerializationConstructor] on more than one constructor; only one constructor can carry it.");
        }
        var attributed = attributedConstructors.Length == 1 ? attributedConstructors[0] : null;
        ...
        foreach (var candidate in candidates)
        {
            ...
            {
                var slotIndex = keyNames is null ? -1 : FindParameterSlot(keyNames, parameters[i]);
                if (slotIndex < 0)
                {
        ...
        if (attributed is not null)
        {
            ...
            throw new MessagePackSerializationException($"'{typeof(T).FullName}': every [SerializationConstructor] parameter must match a serialized member by [Key] name or member name (case-insensitive) and assignable type.");
        }
    ...
    T CreateInstance()
    {
        ...
        {
            throw new MessagePackSerializationException($"Type '{typeof(T).FullName}' has no usable constructor: neither a parameterless one nor an accessible one whose parameters all match serialized members by [Key] name or member name (case-insensitive); v3 bound int-keyed parameters by position, v4 binds by name only. Deserialize into an existing instance instead.");
        }
