...
public sealed partial class ObjectFallbackFormatter<TWriteBuffer, TReadBuffer> : IMessagePackFormatter<TWriteBuffer, TReadBuffer, object?>
{
    ...
    readonly TypeKeyHashTable<Dispatcher> dispatchers = new(); // TypeKeyHashTable: the serializer's add-only Type-keyed cache (open addressing, copy-on-write adds, lock-free reads), see TypeKeyTableBenchmark
