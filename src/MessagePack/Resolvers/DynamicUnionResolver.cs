...
namespace MessagePack.Resolvers
{
    ...
    public class DynamicUnionResolver : IFormatterResolver
    {
        ...
        static TypeInfo BuildType(Type type)
        {
            ...
            var unionAttrs = ti.GetCustomAttributes<UnionAttribute>().ToArray();
        ...
        */
        ...
        static readonly ConstructorInfo typeMapDictionaryConstructor = typeof(Dictionary<RuntimeTypeHandle, KeyValuePair<int, int>>).GetTypeInfo().DeclaredConstructors.First(x => { var p = x.GetParameters(); return p.Length == 1 && p[0].ParameterType == typeof(int); });
        ...
        static readonly MethodInfo typeMapDictionaryTryGetValue = typeof(Dictionary<RuntimeTypeHandle, KeyValuePair<int, int>>).GetRuntimeMethod("TryGetValue", new[] { typeof(RuntimeTypeHandle), refInt });
