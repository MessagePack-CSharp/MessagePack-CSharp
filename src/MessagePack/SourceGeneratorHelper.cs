...
public static class SourceGeneratorHelper
{
    ...
    /// <summary>Throws the duplicate-map-key <see cref="MessagePackSerializationException"/> for a string-keyed member seen twice in one map.</summary>
    // one call site per declared member in every generated map reader; the shared thrower keeps the string literal
    // and the newobj out of each key leaf
    [EditorBrowsable(EditorBrowsableState.Never)]
    [DoesNotReturn]
    public static void ThrowDuplicateMapKey() => MessagePackSerializationException.ThrowDuplicateMapKey();

    /// <summary>Throws the truncated-payload <see cref="MessagePackSerializationException"/> for a string map key whose bytes are not all present.</summary>
    // the generated key loop compares utf8 in place, so a TryGetSpan miss is the one truncation the foundation
    // does not report itself; one call site per generated map reader
    [EditorBrowsable(EditorBrowsableState.Never)]
    [DoesNotReturn]
    public static void ThrowUnexpectedEndOfMapKey() => throw new MessagePackSerializationException("Unexpected end of data while reading a map key.");
