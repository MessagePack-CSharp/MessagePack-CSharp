...
public static partial class MessagePackSerializer
{
    ...
    public static void Deserialize<T>(ReadOnlySpan<byte> source, ref T value, MessagePackSerializerOptions options)
    {
        ...
        if (processor != null && processor.TryDecode(source, out var decoded))
        {
            try
            {
                DeserializeDecoded(ref value, in decoded, source, options);
            }
            finally
            {
                decoded.Dispose();
    ...
    static void DeserializeDecoded<T>(ref T value, in DecodedMessage decoded, ReadOnlySpan<byte> source, MessagePackSerializerOptions options)
    {
        switch (decoded)
        {
    ...
    static void DeserializeDecoded<T>(ref T value, in DecodedMessage decoded, in ReadOnlySequence<byte> source, MessagePackSerializerOptions options)
    {
        switch (decoded)
        {
            case DecodedMessage.SourceSlice slice:
    ...
    static void CheckSourceSlice(DecodedMessage.SourceSlice slice, long sourceLength)
    {
        if ((long)slice.Offset + slice.Length > sourceLength)
        {
            throw new InvalidOperationException($"The message processor returned a source slice ({slice.Offset}, {slice.Length}) outside the {sourceLength}-byte source.");
        }
    }
}
