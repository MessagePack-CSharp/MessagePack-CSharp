...
static class FactoryEmitter
{
    public static string EmitFactory(ImmutableArray<(ObjectModel? Object, UnionModel? Union)> models, ImmutableArray<AttributeFormatterTypeModel> attributeModels, ImmutableArray<SurrogateModel> surrogateModels)
    {
        ...
        using (writer.OpenScope())
        {
            ...
            using (writer.OpenScope())
            {
                // the generic overloads register the serializer's Type-based entry alongside the factory, so Type-first
                // callers (hub protocols, MVC formatters) reach every closed type here on Native AOT without
                // MakeGenericType; open generic definitions have no such entry and take the Type overload
                foreach (var model in orderedAttributes)
                {
                    writer.Line($"SourceGeneratedFormatterFactory.Instance.Register<{model.FullTypeName}>(GeneratedMessagePackFormatterFactory.Instance);");
                }
                foreach (var surrogate in orderedSurrogates)
                {
                    writer.Line($"SourceGeneratedFormatterFactory.Instance.Register<{surrogate.TargetTypeName}>(GeneratedMessagePackFormatterFactory.Instance);");
                }
                foreach (var model in ordered)
                {
                    writer.Line(model.OpenFormatterTypeOf.Length == 0
                        ? $"SourceGeneratedFormatterFactory.Instance.Register<{model.FullTypeName}>(GeneratedMessagePackFormatterFactory.Instance);"
                        : $"SourceGeneratedFormatterFactory.Instance.Register(typeof({model.OpenTypeOf}), GeneratedMessagePackFormatterFactory.Instance);");
                }
                foreach (var model in orderedUnions)
                {
                    writer.Line(model.OpenFormatterTypeOf.Length == 0
                ...
                // harvested closed forms are incidental defaults: they must not displace a declaration (surrogate,
                // attribute) another assembly registers for the same type
                foreach (var instantiation in harvestedInstantiations)
                {
                    writer.Line($"SourceGeneratedFormatterFactory.Instance.RegisterHarvested<{instantiation.ClosedTypeName}>(GeneratedMessagePackFormatterFactory.Instance);");
                }
                foreach (var builtIn in harvestedBuiltIns)
                {
                    writer.Line($"SourceGeneratedFormatterFactory.Instance.RegisterHarvested<{builtIn.ClosedTypeName}>(GeneratedMessagePackFormatterFactory.Instance);");
                }
            }
        }
    ...
    public static string EmitSerializableFactory(SerializableFactoryModel model, ImmutableArray<(ObjectModel? Object, UnionModel? Union)> models)
    {
        ...
        if (registrations.Length > 0)
        {
            ...
            using (writer.OpenScope())
            {
                ...
                {
                    writer.Line($"global::MessagePack.SourceGeneratedFormatterFactory.Instance.RegisterHarvested<{registration}>(Instance);");
                }
            }
        }
