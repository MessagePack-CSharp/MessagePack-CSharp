...
/// used to emit the Type-based CreateFormatter dispatch, the downlevel resolver's only path.
/// Update here when a first-party fallback buffer ships (the async pipe buffers join when async entries land),
/// every generated factory picks the change up on rebuild.
///
/// On `allows ref struct` TFMs the emitted member returns null ("not mine")
/// without the dispatch: the generic member is implemented directly there, the resolver calls it,
/// and the bridge only serves types that do not implement the generic member,
/// so the dispatch would be dead code whose static `new Formatter&lt;ConcretePair&gt;()` references force every
/// formatter x every pair instantiation into Native AOT binaries (value-type buffer args get no __Canon sharing).
/// It must be null and not a throw: the resolver's missing-formatter probe and composite passthroughs route Type-based
/// calls through modern factories, and a throw there would abort the whole chain.
/// </summary>
internal static class BufferPairs
{
    // (writeBuffer, readBuffer) fully-qualified names; the preprocessor split below is evaluated per consuming TFM
    // (generated trees share the compilation's parse options)
    ...
    /// <summary>
    /// Appends the full non-generic CreateFormatter implementation: fallback-pair dispatch into the type's own generic
