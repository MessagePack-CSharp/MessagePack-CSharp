...
#pragma warning restore SA1509 // Opening braces should not be preceded by blank line
            ...
            foreach (IPropertySymbol item in instanceMembers.OfType<IPropertySymbol>())
            {
                ...
                else
                {
                    ...
                    if (isIntKey)
                    {
                        ...
                        var member = new MemberSerializationInfo(true, isWritable, isReadable, isInitOnly, item.IsRequired, intKey!.Value, item.Name, item.Name, item.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), item.Type.ToDisplayString(BinaryWriteFormat), IsByRefDispatchableValueType(item.Type), CanDeserializeInto(item.Type), specialFormatter)
                        {
                            DeclaringType = GetDeclaringTypeIfColliding(item),
                        };
    ...
    private static bool CanDeserializeInto(ITypeSymbol symbol)
    {
