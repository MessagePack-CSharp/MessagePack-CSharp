...
static class AttributeFormatterParser
{
    ...
    public static AttributeFormatterParseResult Parse(GeneratorAttributeSyntaxContext context)
    {
        ...
        for (var accessible = type; accessible is not null; accessible = accessible.ContainingType)
        {
            ...
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.TypeLevelFormatterNotGenerated, $"'{typeName}': type-level [MessagePackFormatter] needs the type (and every containing type) to be public or internal; register the factory in the resolver chain instead.", typeLocation));
                return Empty(diagnostics);
            }
        }
        foreach (var attribute in type.GetAttributes())
        {
            ...
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.TypeLevelFormatterNotGenerated, $"'{typeName}': SuppressSourceGeneration also suppresses the type-level [MessagePackFormatter] registration; register the factory in the resolver chain instead.", typeLocation));
                return Empty(diagnostics);
            }
        }
