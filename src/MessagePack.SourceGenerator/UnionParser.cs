...
static class UnionParser
{
    ...
    public static UnionParseResult Parse(GeneratorAttributeSyntaxContext context)
    {
        ...
        if (ObjectParser.ReadNamedBool(context.Attributes[0], "AllowCircularReferences"))
        {
            ...
            diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidCircularReference, $"'{typeName}': AllowCircularReferences is not supported on union roots yet; annotate the concrete case types instead.", typeLocation));
            return new UnionParseResult(null, new EquatableArray<DiagnosticInfo>([.. diagnostics]));
        }
        ...
        else if (type.TypeKind is TypeKind.Struct or TypeKind.Class)
        {
            ...
            if (!isPatternUnion)
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': [UnionTag] on a concrete type requires a union: a C# union declaration, or a type carrying the [Union] pattern.", typeLocation));
            ...
            if (FindUnionMemberProvider(type) is { } provider)
            {
                ...
                {
                    diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': the IUnionMembers union member provider must be public and implemented by the union type.", typeLocation));
                    return Invalid(diagnostics);
                }
            ...
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': a union root must expose a public Value property (on the type or its IUnionMembers provider) for the generated formatter to read the held case.", typeLocation));
                return Invalid(diagnostics);
            }
        }
        else
        ...
        if (type.IsGenericType)
        {
            ...
            if (!isPatternUnion)
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': generic [UnionTag] roots are supported only for unions (a C# union declaration or a [Union]-pattern type).", typeLocation));
                return Invalid(diagnostics);
            }
        ...
        foreach (var attribute in type.GetAttributes())
        {
            ...
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': [UnionTag] case types must use the typeof overload (closed, or unbound matching a declared case), UnionTag<TCaseType>, or a type-parameter name of a generic union, so the generator can resolve them.", typeLocation));
                valid = false;
                continue;
            }
            if (!isPatternUnion && caseType is INamedTypeSymbol { IsGenericType: true })
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': generic [UnionTag] case type '{caseType.ToDisplayString()}' is not supported yet.", typeLocation));
                valid = false;
                continue;
            }
            ...
            if (!seenTags.Add(tag))
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': [UnionTag] tag {tag} is declared more than once.", typeLocation));
                valid = false;
                continue;
            }
            if (!seenCaseTypes.Add(caseTypeName))
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.InvalidUnion, $"'{typeName}': [UnionTag] case type '{caseType.ToDisplayString()}' is declared more than once.", typeLocation));
                valid = false;
                continue;
            }
