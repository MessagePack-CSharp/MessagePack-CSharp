...
static class SurrogateParser
{
    ...
    public static SurrogateParseResult? Parse(GeneratorSyntaxContext context, CancellationToken cancellationToken)
    {
        ...
        {
            diagnostics.Add(new DiagnosticInfo(Diagnostics.SurrogateNotRegistered, $"'{typeName}': a generic surrogate cannot be auto-registered (the factory needs a closed instantiation); compose SurrogateFormatterFactory into an explicit chain instead.", typeLocation));
            return Result(sites, diagnostics);
        }
        if (!ObjectParser.IsAccessibleToGeneratedCode(type, compilation))
        {
            diagnostics.Add(new DiagnosticInfo(Diagnostics.SurrogateNotRegistered, $"'{typeName}': the surrogate type must be public or internal for the generated factory to construct it; compose SurrogateFormatterFactory into an explicit chain instead.", typeLocation));
            return Result(sites, diagnostics);
        }
        ...
        foreach (var implemented in targets)
        {
            ...
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.SurrogateNotRegistered, $"'{typeName}': the surrogate target '{targetDisplay}' is not a closed type, so it cannot be auto-registered.", typeLocation));
                continue;
            }
            if (!ObjectParser.IsAccessibleToGeneratedCode(target, compilation))
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.SurrogateNotRegistered, $"'{typeName}': the surrogate target '{targetDisplay}' must be public or internal for the generated factory to serve it; compose SurrogateFormatterFactory into an explicit chain instead.", typeLocation));
                continue;
            }
            ...
            {
                diagnostics.Add(new DiagnosticInfo(Diagnostics.SurrogateNotRegistered, $"'{typeName}': the surrogate target '{targetDisplay}' carries its own serialization declaration ([MessagePackObject] or a type-level [MessagePackFormatter]), which wins; the surrogate is not registered. Remove the target's declaration to serialize it through the surrogate.", typeLocation));
                continue;
            }
