    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(surrogateSites, static (spc, sites) =>
        {
                ...
                .GroupBy(static site => site.TargetTypeName, StringComparer.Ordinal))
            {
                ...
                {
                    spc.ReportDiagnostic(new DiagnosticInfo(Diagnostics.DuplicateSurrogate, $"'{group.Key}' has multiple surrogates ({surrogateNames}): the wire shape would be ambiguous. Keep exactly one IMessagePackSurrogate implementation per target.", site.Location).ToDiagnostic());
                }
            }
        });
