...
public class TryReadUInt16Benchmark
{
    ...
    public void Setup()
    {
        ...
        {
            MessagePack.MessagePackPrimitives.TryWriteUInt16(buffer.AsSpan(offset), Next(), out var written);
            offsets[n] = offset;
            lengths[n] = written;
            offset += written;
        }
        ...
        for (int n = 0; n < Count; n++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadUInt16(buffer.AsSpan(o0), out var v0, out var c0);
            var r1 = V3::MessagePack.MessagePackPrimitives.TryReadUInt16(buffer.AsSpan(o1), out var v1, out var c1);
        ...
        if (MessagePack.MessagePackPrimitives.TryReadByte(probe, out _, out var tb) != MessagePack.DecodeResult.TokenMismatch || tb != 0)
        {
            throw new InvalidOperationException("verify failed: byte overflow");
        }
        probe[0] = 0xe0; // -32
        if (MessagePack.MessagePackPrimitives.TryReadSByte(probe, out var sv, out var ts) != MessagePack.DecodeResult.Success || sv != -32 || ts != 1)
        {
            throw new InvalidOperationException("verify failed: sbyte negative fixint");
        }
        if (MessagePack.MessagePackPrimitives.TryReadUInt16(probe, out _, out var tu) != MessagePack.DecodeResult.TokenMismatch || tu != 0)
        {
        ...
        if (MessagePack.MessagePackPrimitives.TryReadByte(probe, out var wb, out var wt) != MessagePack.DecodeResult.Success || wb != 200 || wt != 9)
        {
        ...
        {
            if (MessagePack.MessagePackPrimitives.TryReadInt16(probe[..cut], out _, out var tc) != MessagePack.DecodeResult.InsufficientBuffer || tc != 3)
            {
    ...
    public uint V4()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadUInt16(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public uint V4Exact()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadUInt16(buf.Slice(offsets[n], lengths[n]), out var v, out _) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
        }
    ...
    public uint Mpcs()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (V3::MessagePack.MessagePackPrimitives.TryReadUInt16(buf.Slice(offset), out var v, out var consumed) == V3::MessagePack.MessagePackPrimitives.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
