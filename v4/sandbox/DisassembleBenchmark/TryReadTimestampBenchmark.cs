...
public class TryReadTimestampBenchmark
{
    ...
    public void Setup()
    {
        ...
        {
            expected[n] = Next();
            MessagePack.MessagePackPrimitives.TryWriteTimestamp(buffer.AsSpan(offset), expected[n], out var written);
            offset += written;
        }
        ...
        for (int n = 0; n < Count; n++)
        {
            var r = MessagePack.MessagePackPrimitives.TryReadTimestamp(buffer.AsSpan(o), out var v, out var c);
            if (r != MessagePack.DecodeResult.Success || v != expected[n] || v.Kind != DateTimeKind.Utc)
            {
        ...
        MessagePack.MessagePackPrimitives.TryWriteTimestamp(probe, ts96, out var w96);
        if (w96 != 15 || MessagePack.MessagePackPrimitives.TryReadTimestamp(probe, out var v96, out var t96) != MessagePack.DecodeResult.Success
            || v96 != ts96 || t96 != 15)
        {
        ...
        {
            var r = MessagePack.MessagePackPrimitives.TryReadTimestamp(probe[..cut], out _, out var tc);
            if (r != MessagePack.DecodeResult.InsufficientBuffer || tc <= cut || tc > 15)
            {
                throw new InvalidOperationException($"verify failed: truncation cut={cut}");
            }
        }
        probe[2] = 42; // ext8(12) with a non-timestamp type code
        if (MessagePack.MessagePackPrimitives.TryReadTimestamp(probe, out _, out var tm) != MessagePack.DecodeResult.TokenMismatch || tm != 0)
        {
    ...
    public long V4()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadTimestamp(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v.Ticks;
            }
            offset += consumed;
