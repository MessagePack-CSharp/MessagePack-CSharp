...
//   V4     - the current library table (branchless, chain-bound)
...
// Mixed 2.34 (vs V4 1.00 flat; it even beat Mpcs everywhere). BUT the end-to-end
...
public class TryReadInt32RematchBenchmark
{
    ...
    public void Setup()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadInt32(buffer.AsSpan(o0), out var v0, out var c0);
            var r1 = V3::MessagePack.MessagePackPrimitives.TryReadInt32(buffer.AsSpan(o1), out var v1, out var c1);
            ...
            if (r0 != MessagePack.DecodeResult.Success || r1 != V3::MessagePack.MessagePackPrimitives.DecodeResult.Success
                || r2 != MessagePack.DecodeResult.Success || r3 != MessagePack.DecodeResult.Success
        ...
        {
            if (MessagePackPrimitives.TryReadInt32CascadeDr(probe[..cut], out _, out _) != MessagePack.DecodeResult.InsufficientBuffer
                || MessagePackPrimitives.TryReadInt32HybridDr(probe[..cut], out _, out _) != MessagePack.DecodeResult.InsufficientBuffer)
            {
                throw new InvalidOperationException($"verify failed: truncation cut={cut}");
            }
        }
        probe[0] = 0xa0; // fixstr: not an int
        if (MessagePackPrimitives.TryReadInt32CascadeDr(probe, out _, out _) != MessagePack.DecodeResult.TokenMismatch
            || MessagePackPrimitives.TryReadInt32HybridDr(probe, out _, out _) != MessagePack.DecodeResult.TokenMismatch)
        {
    ...
    public int V4()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadInt32(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public int Mpcs()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (V3::MessagePack.MessagePackPrimitives.TryReadInt32(buf.Slice(offset), out var v, out var consumed) == V3::MessagePack.MessagePackPrimitives.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public int CascadeDr()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePackPrimitives.TryReadInt32CascadeDr(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public int HybridDr()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePackPrimitives.TryReadInt32HybridDr(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
