...
//
// MEASURED (Zen 5, ShortRun; V4 cascade -> V4 table, Mpcs for context):
...
public class TryReadUInt64Benchmark
{
    ...
    public void Setup()
    {
        ...
        {
            MessagePack.MessagePackPrimitives.TryWriteUInt64(buffer.AsSpan(offset), Next(), out var written);
            offset += written;
        }
        ...
        for (int n = 0; n < Count; n++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadUInt64(buffer.AsSpan(o0), out var v0, out var c0);
            var r1 = V3::MessagePack.MessagePackPrimitives.TryReadUInt64(buffer.AsSpan(o1), out var v1, out var c1);
        ...
        if (MessagePack.MessagePackPrimitives.TryReadUInt64(probe, out _, out var tn) != MessagePack.DecodeResult.TokenMismatch || tn != 0)
        {
            throw new InvalidOperationException("verify failed: negative fixint");
        }
        probe[0] = 0xd0; // int8
        probe[1] = 0xff; // -1
        if (MessagePack.MessagePackPrimitives.TryReadUInt64(probe, out _, out var ti) != MessagePack.DecodeResult.TokenMismatch || ti != 0)
        {
        ...
        {
            if (MessagePack.MessagePackPrimitives.TryReadUInt64(probe[..cut], out _, out var tc) != MessagePack.DecodeResult.InsufficientBuffer || tc != 9)
            {
    ...
    public ulong V4()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadUInt64(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public ulong Mpcs()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (V3::MessagePack.MessagePackPrimitives.TryReadUInt64(buf.Slice(offset), out var v, out var consumed) == V3::MessagePack.MessagePackPrimitives.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
