...
public static class LadderSuperlaneCandidates
{
    ...
    public static bool ReadInt16Scalar(ReadOnlySpan<byte> s, Span<short> dst)
    {
        ...
        {
            if (UltraMessagePack.MessagePackPrimitives.TryReadInt16(s.Slice(o), out dst[i], out var size) != UltraMessagePack.DecodeResult.Success)
            {
                return false;
            }
            o += size;
    ...
    public static bool ReadInt16Superlane(ReadOnlySpan<byte> s, Span<short> dst)
    {
        ...
        if (Vector128.IsHardwareAccelerated)
        {
            while (i + 16 <= count)
            {
                ...
                for (; i < end; i++)
                {
                    if (UltraMessagePack.MessagePackPrimitives.TryReadInt16(s.Slice(o), out Unsafe.Add(ref dr, i), out var size) != UltraMessagePack.DecodeResult.Success)
                    {
                        return false;
                    }
                    o += size;
    ...
    public static bool ReadInt64Scalar(ReadOnlySpan<byte> s, Span<long> dst)
    {
        ...
        for (int i = 0; i < dst.Length; i++)
        {
            if (UltraMessagePack.MessagePackPrimitives.TryReadInt64(s.Slice(o), out dst[i], out var size) != UltraMessagePack.DecodeResult.Success)
            {
                return false;
            }
            o += size;
    ...
    public static bool ReadInt64Superlane(ReadOnlySpan<byte> s, Span<long> dst)
    {
        ...
        if (Vector512.IsHardwareAccelerated && Avx512F.IsSupported)
        {
            while (i + 8 <= count)
            {
                ...
                for (; i < end; i++)
                {
                    if (UltraMessagePack.MessagePackPrimitives.TryReadInt64(s.Slice(o), out Unsafe.Add(ref dr, i), out var size) != UltraMessagePack.DecodeResult.Success)
                    {
                        return false;
                    }
                    o += size;
