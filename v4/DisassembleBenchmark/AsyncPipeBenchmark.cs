...
//   Ultra           = 2-pass (resumable boundary scan, then sync parse of the slice)
...
public class AsyncPipeBenchmark
{
    ...
    public void Setup()
    {
        ...
        var single = MessagePack.MessagePackSerializer.Serialize(poco, MessagePack.MessagePackSerializerOptions.Standard);
        messagesPayload = Enumerable.Repeat(single, MessageCount).SelectMany(b => b).ToArray();
        arrayPayload = MessagePack.MessagePackSerializer.Serialize(
            Enumerable.Repeat(poco, MessageCount).ToArray(), MessagePack.MessagePackSerializerOptions.Standard);

        // async paths are outside --verify, so self-check here: every competitor must
        // observe every message
        Check(nameof(Messages_Ultra), Messages_Ultra);
        Check(nameof(Messages_MsgPackCSharp), Messages_MsgPackCSharp);
        Check(nameof(Messages_Nerdbank), Messages_Nerdbank);
        Check(nameof(Array_Ultra), Array_Ultra);
        Check(nameof(Array_MsgPackCSharp), Array_MsgPackCSharp);
        Check(nameof(Single_Ultra), Single_Ultra);
    ...
    public async Task<int> Messages_MsgPackCSharp()
    {
        ...
        var count = 0;
        using (var streamReader = new MessagePack.MessagePackStreamReader(reader.AsStream()))
        {
            while (await streamReader.ReadAsync(CancellationToken.None) is ReadOnlySequence<byte> message)
            {
                MessagePack.MessagePackSerializer.Deserialize<NbPocoAsArray>(message, MessagePack.MessagePackSerializerOptions.Standard);
                count++;
            }
        }
        await writer;
    ...
    public async Task<int> Messages_Ultra()
    {
        var reader = StartFeed(messagesPayload, out var writer);
        var count = 0;
        await foreach (var item in UltraMessagePack.MessagePackSerializer.DeserializeMessagesAsync<NbPocoAsArray>(reader))
        {
            count++;
        }
        await writer;
    ...
    public async Task<int> Array_MsgPackCSharp()
    {
        ...
        using (var streamReader = new MessagePack.MessagePackStreamReader(reader.AsStream()))
        {
            ...
            for (var i = 0; i < length; i++)
            {
                ...
                MessagePack.MessagePackSerializer.Deserialize<NbPocoAsArray>(element, MessagePack.MessagePackSerializerOptions.Standard);
                count++;
            }
        }
        await writer;
    ...
    public async Task<int> Array_Ultra()
    {
        var reader = StartFeed(arrayPayload, out var writer);
        var count = 0;
        await foreach (var item in UltraMessagePack.MessagePackSerializer.DeserializeElementsAsync<NbPocoAsArray>(reader))
        {
            count++;
        }
        await writer;
    ...
    {
        var reader = StartFeed(arrayPayload, out var writer);
        var value = await MessagePack.MessagePackSerializer.DeserializeAsync<NbPocoAsArray[]>(reader.AsStream(), MessagePack.MessagePackSerializerOptions.Standard);
        await writer;
        return value!.Length;
    }

    [BenchmarkCategory("single"), Benchmark]
    public async Task<int> Single_Ultra()
    {
        var reader = StartFeed(arrayPayload, out var writer);
        var value = await UltraMessagePack.MessagePackSerializer.DeserializeAsync<NbPocoAsArray[]>(reader);
        await writer;
        return value.Length;
    }
