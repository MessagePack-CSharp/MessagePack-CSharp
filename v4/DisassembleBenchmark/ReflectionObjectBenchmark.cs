...
public class ReflectionObjectBenchmark
{
    ...
    public void Setup()
    {
        ...
        // v3's runtime IL.Emit path, forced explicitly: the mpcs source generator also
        // runs on this project, so the plain StandardResolver would not prove Emit
        v3Emit = MessagePack.MessagePackSerializerOptions.Standard.WithResolver(
            MessagePack.Resolvers.CompositeResolver.Create(
                MessagePack.Resolvers.BuiltinResolver.Instance,
                MessagePack.Resolvers.DynamicObjectResolver.Instance));

        payload = UltraMessagePack.MessagePackSerializer.Serialize(person);

        // verify: all three paths must be byte-identical (same array wire) and roundtrip
        if (!UltraMessagePack.MessagePackSerializer.Serialize(person, reflection).AsSpan().SequenceEqual(payload)) throw new InvalidOperationException("verify failed: reflection bytes vs generated");
        if (!MessagePack.MessagePackSerializer.Serialize(person, v3Emit).AsSpan().SequenceEqual(payload)) throw new InvalidOperationException("verify failed: v3 emit bytes vs generated");
        var back = UltraMessagePack.MessagePackSerializer.Deserialize<BenchPerson>(payload, reflection)!;
        ...
        var v3Back = MessagePack.MessagePackSerializer.Deserialize<BenchPerson>(payload, v3Emit);
    ...
    public byte[] SerializeReflection() => UltraMessagePack.MessagePackSerializer.Serialize(person, reflection);

    [BenchmarkCategory("Serialize"), Benchmark]
    public byte[] SerializeV3Emit() => MessagePack.MessagePackSerializer.Serialize(person, v3Emit);
    ...
    public BenchPerson DeserializeReflection() => UltraMessagePack.MessagePackSerializer.Deserialize<BenchPerson>(payload, reflection)!;

    [BenchmarkCategory("Deserialize"), Benchmark]
    public BenchPerson DeserializeV3Emit() => MessagePack.MessagePackSerializer.Deserialize<BenchPerson>(payload, v3Emit);
