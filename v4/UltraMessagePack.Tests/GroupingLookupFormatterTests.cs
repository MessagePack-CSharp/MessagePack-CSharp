...
public class GroupingLookupFormatterTests
{
    ...
    // the v4-wide last-win policy applies (v3's Dictionary.Add would have thrown)
    [Fact]
    public void DuplicateGroupKeys_LastWins()
    {
        ...
        var back = MessagePackSerializer.Deserialize<ILookup<int, string>>(writer.WrittenSpan.ToArray())!;
        Assert.Equal(1, back.Count);
        Assert.Equal(["second"], back[1]);
    }
}
