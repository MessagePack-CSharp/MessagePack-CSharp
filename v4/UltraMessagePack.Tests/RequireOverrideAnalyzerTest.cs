...
public class RequireOverrideAnalyzerTest
{
    ...
    const string BridgeRidingSource = """
        public sealed class InterfaceOnlyProcessor : UltraMessagePack.MessageProcessor
        {
    ...
    {
        const string source = """
            public sealed class FullProcessor : UltraMessagePack.MessageProcessor
            {
                public override bool TryEncode(int output)
                {
                    return false;
    ...
    public async Task IntermediateBaseOverrides_LeafSilent()
    {
        ...
        const string source = """
            public abstract class Intermediate : UltraMessagePack.MessageProcessor
            {
                public override bool TryEncode<TWriteBuffer>(int message, ref TWriteBuffer output)
                {
                    return false;
