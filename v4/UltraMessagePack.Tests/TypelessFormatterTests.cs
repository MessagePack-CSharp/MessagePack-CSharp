...
public class TypelessFormatterTests
{
    ...
    public void CrossCompatible_WithV3Typeless_BothDirections()
    {
        ...
        foreach (var value in values)
        {
            ...
            var fromV3 = MessagePackSerializer.Deserialize<object?>(
                MessagePack.MessagePackSerializer.Typeless.Serialize(value), options)!;
            Assert.Equal(value.GetType(), fromV3.GetType());
            AssertValueEqual(value, fromV3);

            // UMP bytes -> v3
            var fromUmp = MessagePack.MessagePackSerializer.Typeless.Deserialize(
    ...
    public void CrossCompatible_NestedGraph()
    {
        ...
        var fromUmp = Assert.IsType<ObjectHolder>(MessagePack.MessagePackSerializer.Typeless.Deserialize(
            MessagePackSerializer.Serialize<object?>(value, options)));
    ...
    public void PrimitiveBytes_MatchV3Exactly()
    {
        ...
        {
            Assert.Equal(
                MessagePack.MessagePackSerializer.Typeless.Serialize(value),
                MessagePackSerializer.Serialize<object?>(value, options));
        }
    }
    ...
    public void TypeLoaderHook_IsSoleAuthority()
    {
        ...
        [
            MessagePackFormatterFactory.Combine(
                new TypelessFormatterFactory(TypelessTypeLoader.Create(name => name.StartsWith("UltraMessagePack.Tests.TypelessFormatterTests+TypelessPerson,", StringComparison.Ordinal) ? typeof(TypelessPerson) : null)),
    ...
    public void OmitAssemblyVersion_MatchesV3AndCrossReads()
    {
        ...
        var v3OmitOptions = MessagePack.Resolvers.TypelessContractlessStandardResolver.Options.WithOmitAssemblyVersion(true);
        ...
        Assert.Equal(MessagePack.MessagePackSerializer.Serialize<object>(value, v3OmitOptions), umpBytes);
        Assert.Equal(4, Assert.IsType<TypelessPerson>(MessagePackSerializer.Deserialize<object?>(umpBytes, options)).Age);
        Assert.Equal(4, Assert.IsType<TypelessPerson>(MessagePack.MessagePackSerializer.Typeless.Deserialize(umpBytes)).Age);
    }
