...
namespace MessagePack.Tests
{
    public class MessagePackSerializerTest
    {
        ...
        public void NonGeneric()
        {
            ...
            var data2 = MessagePackSerializer.Deserialize(t, MessagePackSerializer.Serialize(t, data, MessagePackSerializerOptions.Default)) as FirstSimpleData;
            ...
            ms = new MemoryStream();
            MessagePackSerializer.Serialize(t, ms, data, MessagePackSerializerOptions.Default);
            ms.Position = 0;
            var data4 = MessagePackSerializer.Deserialize(t, ms, MessagePackSerializerOptions.Default) as FirstSimpleData;

#if DYNAMIC_GENERATION
            var data5 = data4;
#else
            MessagePackSerializer.Serialize(t, ref writer, data, MessagePackSerializerOptions.Default);
            writer.Flush();
            var reader = new MessagePackReader(writerBytes.AsReadOnlySequence);
            var data5 = MessagePackSerializer.Deserialize(t, ref reader, MessagePackSerializerOptions.Default) as FirstSimpleData;
#endif
        ...
        public void NonGeneric_BufferWriter_ReadOnlySequence()
        {
            ...
            MessagePackSerializer.Serialize(t, writerBytes, data, MessagePackSerializerOptions.Default);
            var data5 = MessagePackSerializer.Deserialize(t, writerBytes.AsReadOnlySequence, MessagePackSerializerOptions.Default) as FirstSimpleData;
...
#if V3COMPAT_EXCLUDED // V3COMPAT-EDIT: hand-built nested payloads via v3 MessagePackWriter; v4 has no public writer struct
        [Fact]
        [Trait("CWE", "674")]
        ...
        [Fact(Skip = "v4 intentional: the depth guard throws MessagePackSerializationException with no fabricated InnerException; v3's inner InsufficientExecutionStackException was an artifact of RuntimeHelpers.EnsureSufficientExecutionStack")]
        public void StackDepthCheck_DynamicObjectResolver()
        {
            ...
            byte[] msgpack = MessagePackSerializer.Serialize(graph, MessagePackSerializerOptions.Default);

            var options = MessagePackSerializerOptions.Default.WithSecurity(MessagePackSecurity.UntrustedData.WithMaximumObjectGraphDepth(3));
            MessagePackSerializer.Deserialize<RecursiveObjectGraph>(msgpack, options);

            options = MessagePackSerializerOptions.Default.WithSecurity(MessagePackSecurity.UntrustedData.WithMaximumObjectGraphDepth(2));
        ...
        [Fact(Skip = "v4 intentional: union dispatch is source-generator-only (runtime reflection tier does not consume [UnionTag])")]
        [Trait("CWE", "674")]
...
#if V3COMPAT_EXCLUDED // V3COMPAT-EDIT: helpers built on v3 MessagePackReader/Writer structs
