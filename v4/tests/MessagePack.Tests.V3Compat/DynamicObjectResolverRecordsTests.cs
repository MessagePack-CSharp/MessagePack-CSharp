...
namespace MessagePack.Tests
{
    public class DynamicObjectResolverRecordsTests : TestBase
    {
        ...
        {
            var options = allowPrivate ? StandardResolverAllowPrivate.Options : MessagePackSerializerOptions.Default;
            // V3COMPAT-EDIT: v4's synchronous Serialize/Deserialize take no CancellationToken
            byte[] msgpack = MessagePackSerializer.Serialize(value, options);
            T deserializedValue = MessagePackSerializer.Deserialize<T>(msgpack, options);
