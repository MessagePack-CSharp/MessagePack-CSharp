...
/// and each hit routes to exactly one shape: [UnionTag] present makes the type a union
/// base (<see cref="UnionParser"/>), otherwise it is an object (<see cref="ObjectParser"/>).
/// A [UnionTag] without [MessagePackObject] is invisible here by design; the standalone
/// MsgPack103 analyzer surfaces that mistake. Both model streams combine into one factory +
...
public sealed class MessagePackGenerator : IIncrementalGenerator
{
    ...
    {
        // ModuleInitializerAttribute polyfill decision: the registration file needs the
        // attribute, netstandard2.x / .NET Framework targets do not have it in the BCL, and
        // the compiler recognizes it purely by name (module cctors run on every runtime).
        ...
        // TYPE-LEVEL [MessagePackFormatter]: bound at compile time so the generated factory
        // constructs the attribute-directed formatter directly, and REGISTERED through the
        // same module-initializer path as generated object formatters — the annotation is
        // sugar over the registry tier, not a separate resolution channel (no runtime
        // attribute tier exists). Precedence over a same-type generated object formatter is
        // decided inside FactoryEmitter, mirroring v3's attribute-outranks-generated order.
        // ForAttributeWithMetadataName matches the exact attribute class, so the generic
        // MessagePackFormatterAttribute<TFactory> variant needs its own pipeline
        ...
        // [MessagePackSerializable] root-factory classes (the JsonSerializerContext
        // pattern): each declared root runs through the member-graph harvest, and the
        // generated partial supplies the factory half plus module-initializer
        // registrations — root-only shapes (Person[], List<Person>) resolve on AOT.
        // Exact-name matching again forces one pipeline per attribute arity; results
        // targeting the same class (both forms used) merge in the factory node.
        ...
        // IMessagePackSurrogate implementations: the implementation IS the declaration, so
        // the factory auto-registers each target through a statically closed
        // SurrogateFormatter (an interface has no attribute for FAWMN — a cheap base-list
        // syntax gate feeds the semantic transform instead)
        ...
        // duplicate-target detection is inherently cross-type: collected over the
        // location-carrying sites, separate from the factory node (which must stay free of
        // locations so line-shifting edits do not re-emit it)
        ...
        // Models only — deliberately NOT the whole parse results: diagnostics carry
        // source locations, and collecting them would re-emit the factory on every
        // line-shifting edit near a diagnosed type
        var models = results
        ...
        context.RegisterSourceOutput(models.Combine(attributeModels).Combine(genericAttributeModels).Combine(serializableModels.Combine(genericSerializableModels)).Combine(surrogateModels).Combine(needsModuleInitializerPolyfill), static (spc, pair) =>
        {
            ...
            // a class using BOTH attribute forms produced one result per pipeline: merge
            // per class, unioning the harvested sets (deduped by closed type name)
            ...
            {
                // emitted ONLY alongside a registration file, so compilations that merely
                // reference the generator (no [MessagePackObject]/[MessagePackFormatter]/
                // [MessagePackSerializable] types) never receive the polyfill. A remaining
                // collision is possible when a downlevel compilation ALSO gets the attribute
                // from another polyfill generator (e.g. PolySharp, invisible to
                // CompilationProvider): exclude one side there (PolySharp:
