...
// The attributes are matched by full name because they are usually embedded per assembly
// (there is no shared runtime type to typeof against, and netstandard2.0 has no
// NullabilityInfoContext); one manual reader keeps every TFM on the same code path.
// Scope is deliberately the declared member type only: generic type parameters and the
// element/argument positions inside a generic instantiation are not inspected, which is
// the same hole the source generator documents for nullable validation.
static class NullableAnnotationReader
{
    ...
    public static bool IsNonNullableReference(MemberInfo member, Type memberType)
    {
        ...
        // NullableAttribute on the member wins; without one the member follows the
        // nearest enclosing NullableContext (member, then the declaring type chain)
    ...
    static int ReadNullableFlag(object[] attributes)
    {
        foreach (var attribute in attributes)
        {
            ...
            // both ctor shapes, (byte) and (byte[]), store the blob in NullableFlags;
            // the first element describes the member's top-level type
