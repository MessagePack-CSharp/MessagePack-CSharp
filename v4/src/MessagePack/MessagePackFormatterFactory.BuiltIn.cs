...
public sealed partial class BuiltInFormatterFactory : MessagePackFormatterFactory
{
...
#endif
    {
        ...
        // the retention packet is a member-position marker, not a value type of its own;
        // serving it here (or letting contractless treat it as a POCO) would serialize the
        // container instead of the captured payload it stands for
        if (type == typeof(MessagePackUnknownMembers))
        {
        ...
        if (type == typeof(System.Runtime.InteropServices.NFloat)) return new NFloatFormatter<TWriteBuffer, TReadBuffer>();
#endif

#if NET11_0_OR_GREATER
        // NumericFormatters.cs (.NET 11 additions; Complex<T> rides GenericFormatterFactory
        // and the source generator's built-in harvest)
        if (type == typeof(System.Numerics.BFloat16)) return new BFloat16Formatter<TWriteBuffer, TReadBuffer>();
        if (type == typeof(System.Numerics.Decimal32)) return new Decimal32Formatter<TWriteBuffer, TReadBuffer>();
        if (type == typeof(System.Numerics.Decimal64)) return new Decimal64Formatter<TWriteBuffer, TReadBuffer>();
        if (type == typeof(System.Numerics.Decimal128)) return new Decimal128Formatter<TWriteBuffer, TReadBuffer>();
#endif

        // PrimitiveObjectFormatter.cs / NonGenericCollectionFormatters.cs
        // (ExpandoObject is deliberately absent: opt in with ExpandoObjectFormatterFactory, whose [Obsolete] message explains the quadratic Add cost)
