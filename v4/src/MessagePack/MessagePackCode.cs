...
public static class MessagePackCode
{
    ...
    // MessagePack-CSharp-assigned ext type codes (the spec reserves only the negative
    // range; these are library conventions): 100 = typeless, 99/98 = the MessageProcessor
    // LZ4 pair, 97 = circular-reference back-reference (payload = raw big-endian id in
    // the smallest of 1/2/4 bytes)
    public const sbyte CircularReferenceExtensionTypeCode = 97;
}
