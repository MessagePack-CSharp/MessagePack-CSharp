    ...
    where TKey : notnull
{
    ...
    public void Deserialize(ref TReadBuffer buffer, ref DeserializeState state, ref Dictionary<TKey, TValue>? value)
    {
        ...
        // v3 uses Add, but v4 chose last-win (indexer) here.
        // There should be no compatibility issues with existing data.
#if NET
        for (int i = 0; i < count; i++)
        {
            TKey k = default!;
            kf.Deserialize(ref buffer, ref state, ref k);
            ref var slot = ref CollectionsMarshal.GetValueRefOrAddDefault(result, k, out _);
            slot = default!; // clear slot before deserializing into it
    ...
    where TKey : notnull
{
    ...
    public void Deserialize(ref TReadBuffer buffer, ref DeserializeState state, ref OrderedDictionary<TKey, TValue>? value)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            kf.Deserialize(ref buffer, ref state, ref k);
            vf.Deserialize(ref buffer, ref state, ref v);
            // last-win for the VALUE; a duplicate key keeps its FIRST position
            // (indexer semantics: update-in-place for existing keys, append for new)
            result[k] = v;
        }
