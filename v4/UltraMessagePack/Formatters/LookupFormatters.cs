...
sealed class Grouping<TKey, TElement> : IGrouping<TKey, TElement>
{
    ...
    // mutable: a duplicate key in a Lookup payload replaces the contents in place(last-win value, first-encounter position)
    ...
    where TKey : notnull
{
    ...
    public void Deserialize(ref TReadBuffer buffer, ref DeserializeState state, ref ILookup<TKey, TElement>? value)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // duplicate key: last-win for the VALUE, first-encounter position kept —
            // the same rule as OrderedDictionary and the v4 dictionary family
            if (map.TryGetValue(grouping.Key, out var existing))
            {
                existing.elements = grouping.elements;
            }
            else
            {
