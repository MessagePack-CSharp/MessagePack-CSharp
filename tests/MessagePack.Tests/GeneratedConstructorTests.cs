...
public class GeneratedConstructorTests
{
    ...
    // while its mpc matched declared names only; the generator follows the runtime rule
    [Fact]
    public void StringKeyConstructor_MatchesByKeyName()
    {
        AssertBytesAndRoundtrip(new GenKeyNameCtorPoco(11, "key"));
        var back = V4.Deserialize<GenKeyNameCtorPoco>(V4.Serialize(new GenKeyNameCtorPoco(11, "key")))!;
        Assert.Equal(11, back.Id);
        Assert.Equal("key", back.Name);
    }

    [Fact]
    public void StringKeyConstructor_MixesKeyNamesAndMemberNames()
    {
        AssertBytesAndRoundtrip(new GenMixedKeyNameCtorPoco(7, 8, 9));
        var back = V4.Deserialize<GenMixedKeyNameCtorPoco>(V4.Serialize(new GenMixedKeyNameCtorPoco(7, 8, 9)))!;
        Assert.Equal(7, back.X);
        Assert.Equal(8, back.Y);
        Assert.Equal(9, back.Z);
    }

    [Fact]
    public void StringKeyConstructor_KeyNameWinsOverMemberName()
    {
        // the key names cross the member names: binding by member name would swap the values
        AssertBytesAndRoundtrip(new GenCrossedKeyNameCtorPoco(1, 2));
        var back = V4.Deserialize<GenCrossedKeyNameCtorPoco>(V4.Serialize(new GenCrossedKeyNameCtorPoco(1, 2)))!;
        Assert.Equal(2, back.A);
...
[V3::MessagePack.MessagePackObject]
public class GenKeyNameCtorPoco
{
    [V3::MessagePack.Key("my_id")] public int Id { get; }
    [V3::MessagePack.Key("display")] public string? Name { get; }

    public GenKeyNameCtorPoco(int my_id, string? display)
    {
        Id = my_id;
        Name = display;
    }
...
[V3::MessagePack.MessagePackObject]
public class GenMixedKeyNameCtorPoco
{
    [V3::MessagePack.Key("x_val")] public int X { get; }
    [V3::MessagePack.Key("y")] public int Y { get; }
    [V3::MessagePack.Key("Z")] public int Z { get; }

    // x binds by member name, y and z by key name (case-insensitive)
    public GenMixedKeyNameCtorPoco(int x, int y, int z)
    {
        X = x;
        Y = y;
        Z = z;
...
[V3::MessagePack.MessagePackObject]
public class GenCrossedKeyNameCtorPoco
{
    [V3::MessagePack.Key("b")] public int A { get; }
    [V3::MessagePack.Key("a")] public int B { get; }

    // parameter a carries key "a" (member B), parameter b carries key "b" (member A)
    public GenCrossedKeyNameCtorPoco(int a, int b)
    {
        A = b;
        B = a;
    }
