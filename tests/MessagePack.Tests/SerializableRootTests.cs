...
public class SerializableRootTests
{
    ...
    // a root that closes nothing on its own is a misunderstanding of the attribute, not a silent empty table
    [Fact]
    public void PrimitiveArrayRoot_ReportsMsgPack022()
    {
        var result = RunGenerator("""
            using MessagePack;
            [MessagePackSerializable<int[]>]
            ...
            """);
        var diagnostic = Assert.Single(result.Diagnostics, d => d.Id == "MsgPack022");
        Assert.Equal(DiagnosticSeverity.Warning, diagnostic.Severity);
        Assert.Contains("int[]", diagnostic.GetMessage());
        Assert.Equal(1, diagnostic.Location.GetLineSpan().StartLinePosition.Line); // on the attribute, not the class
    }

    [Fact]
    public void SelfDeclaredObjectRoot_ReportsMsgPack022WithTheCollectionHint()
    {
        var result = RunGenerator("""
            using MessagePack;
            [MessagePackObject]
            ...
            """);
        var diagnostic = Assert.Single(result.Diagnostics, d => d.Id == "MsgPack022");
        Assert.Contains("already registers it", diagnostic.GetMessage());
        Assert.Contains("Model[]", diagnostic.GetMessage());
    }

    [Fact]
    public void UndeclaredClassRoot_ReportsMsgPack022()
    {
        var result = RunGenerator("""
            using MessagePack;
            public class Plain { public int X { get; set; } }
            [MessagePackSerializable<Plain>]
            public partial class Roots
            {
            }
            """);
        var diagnostic = Assert.Single(result.Diagnostics, d => d.Id == "MsgPack022");
        Assert.Contains("not made serializable by it", diagnostic.GetMessage());
    }

    // a duplicate of an earlier root is deduplicated, not warned: its closure is non-empty on its own
    [Fact]
    public void CollectionRootsDoNotReportMsgPack022()
    {
        var result = RunGenerator("""
            using MessagePack;
            using System.Collections.Generic;
            ...
            [MessagePackSerializable<Model[]>]
            [MessagePackSerializable<Model[]>]
            [MessagePackSerializable<List<Model>>]
            [MessagePackSerializable<Model?[]>]
            public partial class Roots
            {
            }
            """);
        Assert.DoesNotContain(result.Diagnostics, d => d.Id == "MsgPack022");
