...
public class GeneratorReflectionParityTests
{
    ...
    // constructor parameters bind by [Key] name first and member name second in both tiers
    // (v3's DynamicObjectResolver rule); the crossed keys would swap the values under a
    // member-name-only binding
    [Fact]
    public void Constructor_BindsByKeyNameBeforeMemberName_BothTiers()
    {
        var bytes = V4.Serialize(new GenCrossedKeyNameCtorPoco(1, 2));
        Assert.Equal(bytes, V4.Serialize(new GenCrossedKeyNameCtorPoco(1, 2), reflectionOptions));
        Assert.Equal(Oracle.Serialize(new GenCrossedKeyNameCtorPoco(1, 2)), bytes);

        var generated = V4.Deserialize<GenCrossedKeyNameCtorPoco>(bytes)!;
        var reflection = V4.Deserialize<GenCrossedKeyNameCtorPoco>(bytes, reflectionOptions)!;
        Assert.Equal((2, 1), (generated.A, generated.B));
        Assert.Equal((2, 1), (reflection.A, reflection.B));

        var byKey = V4.Deserialize<GenKeyNameCtorPoco>(V4.Serialize(new GenKeyNameCtorPoco(11, "key")), reflectionOptions)!;
        Assert.Equal((11, "key"), (byKey.Id, byKey.Name));
    }
