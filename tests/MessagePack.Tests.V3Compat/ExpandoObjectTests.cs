...
namespace MessagePack.Tests
{
    public class ExpandoObjectTests
    {
        ...
        [Fact(Skip = "v4 intentional: ExpandoObject left every default chain; serialization is opt-in via ExpandoObjectFormatterFactory")]
        ...
        public void ExpandoObject_DeepGraphContainsAnonymousType()
        {
            ...
            byte[] bin = MessagePackSerializer.Serialize(expando, MessagePackSerializerOptions.Default);
            this.logger.WriteLine(MessagePackSerializer.ConvertToJson(bin));
        ...
        [Fact(Skip = "v4 intentional: no v3-style security map-size cap; ratified decision (collection reads do not preallocate)")]
        [Trait("CWE", "407")]
        ...
        {
            var options = MessagePackSerializerOptions.Default.WithResolver(TypelessObjectResolver.Instance);
        ...
        private static byte[] CreateMapWithNilValues(int count)
        {
            ...
            return MessagePackSerializer.Serialize(dictionary, MessagePackSerializerOptions.Default);
        }
    }
}
