...
//   V4       - library: one validity branch (always predicted on valid data),
...
//   AllFalse: V4 0.794  Naive 0.799 (1.01x)  Branchless 1.94 (2.44x)
//   AllTrue:  V4 0.796  Naive 0.792 (1.00x)  Branchless 2.04 (2.56x)
//   Random:   V4 0.852  Naive 3.28  (3.84x)  Branchless 2.03 (2.38x)
// asm confirmed the three shapes: V4 = biased cmp + ja (validity, always predicted)
...
public class TryReadBooleanBenchmark
{
    ...
    public void Setup()
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadBoolean(buffer.AsSpan(i), out var v0, out var t0);
            ...
            if (r0 != MessagePack.DecodeResult.Success || r1 != r0 || r2 != r0
        ...
        if (MessagePack.MessagePackPrimitives.TryReadBoolean(default, out _, out var te0) != MessagePack.DecodeResult.InsufficientBuffer || te0 != 1
            || MessagePackPrimitives.TryReadBooleanNaiveSwitch(default, out _, out var te1) != MessagePack.DecodeResult.InsufficientBuffer || te1 != 1
            || MessagePackPrimitives.TryReadBooleanBranchless(default, out _, out var te2) != MessagePack.DecodeResult.InsufficientBuffer || te2 != 1)
        {
            throw new InvalidOperationException("verify failed: empty");
        }
        ReadOnlySpan<byte> bad = [0xa0];
        if (MessagePack.MessagePackPrimitives.TryReadBoolean(bad, out _, out var tb0) != MessagePack.DecodeResult.TokenMismatch || tb0 != 0
            || MessagePackPrimitives.TryReadBooleanNaiveSwitch(bad, out _, out var tb1) != MessagePack.DecodeResult.TokenMismatch || tb1 != 0
            || MessagePackPrimitives.TryReadBooleanBranchless(bad, out _, out var tb2) != MessagePack.DecodeResult.TokenMismatch || tb2 != 0)
        {
    ...
    public int V4()
    {
        ...
        {
            if (MessagePack.MessagePackPrimitives.TryReadBoolean(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                trues += v ? 1 : 0;
            }
            offset += consumed;
    ...
    public int NaiveSwitch()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePackPrimitives.TryReadBooleanNaiveSwitch(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                trues += v ? 1 : 0;
            }
            offset += consumed;
    ...
    public int Branchless()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePackPrimitives.TryReadBooleanBranchless(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                trues += v ? 1 : 0;
            }
            offset += consumed;
