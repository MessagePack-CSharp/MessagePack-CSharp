...
// Serialize Large ns/elem, in-run:      V4   no-wide-lane   Mpcs   Nerdbank
...
// Deserialize Large ns/elem, in-run:    V4    Mpcs   Nerdbank      (prior V4)
...
public class Int32ArrayFormatterBenchmark
{
    ...
    [BenchmarkCategory("Serialize"), Benchmark]
    public byte[] SerializeMpcs() => V3::MessagePack.MessagePackSerializer.Serialize(data);
    ...
    where TReadBuffer : struct, IReadBuffer, allows ref struct
{
    ...
    public void Serialize(ref TWriteBuffer buffer, ref SerializeState state, int[]? value)
    {
        ...
        while (i < length)
        {
            ...
            ref byte d = ref buffer.GetReference((regionEnd - i) * MessagePack.MessagePackPrimitives.MaxInt32Length);
            int written = 0;
            if (Avx512F.IsSupported)
            {
                while (i + 16 <= regionEnd)
                {
                    ...
                    else
                    {
                        ...
                        for (; i < end; i++)
                        {
                            written += MessagePack.MessagePackPrimitives.UnsafeWriteInt32(ref Unsafe.Add(ref d, written), Unsafe.Add(ref src, i));
                        }
                    }
                }
            }
    ...
    public void Deserialize(ref TReadBuffer buffer, ref DeserializeState state, ref int[]? value)
    {
        ...
        if (Vector128.IsHardwareAccelerated)
        {
            while (i + 16 <= count)
            {
                var window = buffer.GetUnreadSpan();
                if (window.Length >= 16)
                {
