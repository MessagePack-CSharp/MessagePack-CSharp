...
public class TryReadByteBenchmark
{
    ...
    public void Setup()
    {
        ...
        {
            MessagePack.MessagePackPrimitives.TryWriteByte(buffer.AsSpan(offset), Next(), out var written);
            offsets[n] = offset;
            lengths[n] = written;
            offset += written;
        }
        ...
        for (int n = 0; n < Count; n++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadByte(buffer.AsSpan(o0), out var v0, out var c0);
            var r1 = V3::MessagePack.MessagePackPrimitives.TryReadByte(buffer.AsSpan(o1), out var v1, out var c1);
            var r2 = MessagePack.MessagePackPrimitives.TryReadByte(buffer.AsSpan(offsets[n], lengths[n]), out var v2, out var c2);
            if (r0 != MessagePack.DecodeResult.Success || r1 != V3::MessagePack.MessagePackPrimitives.DecodeResult.Success
                || r2 != MessagePack.DecodeResult.Success || v0 != v1 || v0 != v2 || c0 != c1 || c0 != c2)
            {
    ...
    public uint Stream()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadByte(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public uint ExactWindow()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadByte(buf.Slice(offsets[n], lengths[n]), out var v, out _) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
        }
    ...
    public uint Mpcs()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (V3::MessagePack.MessagePackPrimitives.TryReadByte(buf.Slice(offset), out var v, out var consumed) == V3::MessagePack.MessagePackPrimitives.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
