...
// MessagePack.MessagePackPrimitives.TryReadInt32 vs the ecosystem's primitives:
// MessagePack-CSharp v3 (V3::MessagePack.MessagePackPrimitives.TryReadInt32) and
...
//   homogeneous classes: MPCS/Nerdbank 0.82-1.00ns vs V4 flat 2.57ns — cascade WINS 3x
//   Mixed:               MPCS 9.18 / Nerdbank 9.90 vs V4 2.58ns  — V4 WINS 3.5-3.8x
...
public class TryReadInt32CompareBenchmark
{
    ...
    public void Setup()
    {
        ...
        for (int i = 0; i < Count; i++)
        {
            var ru = MessagePack.MessagePackPrimitives.TryReadInt32(buffer.AsSpan(ou), out var vu, out var cu);
            var rm = V3::MessagePack.MessagePackPrimitives.TryReadInt32(buffer.AsSpan(om), out var vm, out var cm);
            var rn = global::Nerdbank.MessagePack.MessagePackPrimitives.TryRead(buffer.AsSpan(on), out int vn, out var cn);
            if (ru != MessagePack.DecodeResult.Success || rm != V3::MessagePack.MessagePackPrimitives.DecodeResult.Success || rn != Nerdbank.MessagePack.MessagePackPrimitives.DecodeResult.Success
                || vu != vm || vu != vn || cu != cm || cu != cn)
            {
                throw new InvalidOperationException($"verify failed at message {i}: v4=({ru},{vu},{cu}) mpcs=({rm},{vm},{cm}) nb=({rn},{vn},{cn})");
            }
            ou += cu;
            om += cm;
            on += cn;
