...
//   Ascii8:  V4 14.0 vs Stack 21.6 ns (candidate 1.55x SLOWER)
...
public class TryReadStringBenchmark
{
    ...
    public void Setup()
    {
        ...
        {
            offset += MessagePack.MessagePackPrimitives.UnsafeWriteString(ref buffer[offset], Next());
        }
        ...
        for (int n = 0; n < Count; n++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadString(buffer.AsSpan(o0), out var v0, out var c0);
            var r1 = TryReadStringStack(buffer.AsSpan(o1), out var v1, out var c1);
            if (r0 != MessagePack.DecodeResult.Success || r1 != r0 || v0 != v1 || c0 != c1)
            {
    ...
    static MessagePack.DecodeResult TryReadStringStack(ReadOnlySpan<byte> source, out string? value, out int tokenSize)
    {
        value = null;
        ...
        var r = MessagePack.MessagePackPrimitives.TryReadStringHeader(source, out int byteCount, out int headerSize);
        if (r != MessagePack.DecodeResult.Success)
        {
    ...
    public int V4()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadString(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v!.Length;
            }
            offset += consumed;
    ...
    public int StackTranscode()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (TryReadStringStack(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v!.Length;
            }
            offset += consumed;
