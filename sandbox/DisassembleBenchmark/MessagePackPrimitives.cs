...
public static class MessagePackPrimitives
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MessagePack.DecodeResult TryReadInt32CascadeDr(ReadOnlySpan<byte> source, out int value, out int bytesConsumed)
    {
        value = 0;
        bytesConsumed = 0;
        if (source.IsEmpty) return MessagePack.DecodeResult.InsufficientBuffer;
        byte code = source[0];
        if (code <= 0x7f) // positive fixint
        {
            value = code;
            bytesConsumed = 1;
            return MessagePack.DecodeResult.Success;
        }
        ...
        {
            case UInt8Code:
                if (source.Length < 2) return MessagePack.DecodeResult.InsufficientBuffer;
                value = source[1];
                bytesConsumed = 2;
                return MessagePack.DecodeResult.Success;
            case UInt16Code:
                if (source.Length < 3) return MessagePack.DecodeResult.InsufficientBuffer;
                value = BinaryPrimitives.ReadUInt16BigEndian(source.Slice(1));
                bytesConsumed = 3;
                return MessagePack.DecodeResult.Success;
            case UInt32Code:
            {
                if (source.Length < 5) return MessagePack.DecodeResult.InsufficientBuffer;
                uint v = BinaryPrimitives.ReadUInt32BigEndian(source.Slice(1));
                if (v > int.MaxValue) return MessagePack.DecodeResult.TokenMismatch;
            ...
            case Int64Code:
            {
                if (source.Length < 9) return MessagePack.DecodeResult.InsufficientBuffer;
                long v = BinaryPrimitives.ReadInt64BigEndian(source.Slice(1));
                value = unchecked((int)v);
                if (value != v) { value = 0; return MessagePack.DecodeResult.TokenMismatch; }
                bytesConsumed = 9;
                return MessagePack.DecodeResult.Success;
            }
            case UInt64Code:
            ...
            default:
                return MessagePack.DecodeResult.TokenMismatch;
        }
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MessagePack.DecodeResult TryReadInt32HybridDr(ReadOnlySpan<byte> source, out int value, out int bytesConsumed)
    {
        if (!source.IsEmpty)
        {
            byte code = source[0];
        ...
        return MessagePack.MessagePackPrimitives.TryReadInt32(source, out value, out bytesConsumed);
    }
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MessagePack.DecodeResult TryReadBooleanNaiveSwitch(ReadOnlySpan<byte> source, out bool value, out int tokenSize)
    {
        tokenSize = 1;
        if (source.IsEmpty)
        {
            value = false;
            return MessagePack.DecodeResult.InsufficientBuffer;
        }
        switch (source[0])
        {
            case 0xc3:
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static MessagePack.DecodeResult TryReadBooleanBranchless(ReadOnlySpan<byte> source, out bool value, out int tokenSize)
    {
        if (source.IsEmpty)
        {
            value = false;
        ...
        value = d == 1;
        tokenSize = ok ? 1 : 0;
        return ok ? MessagePack.DecodeResult.Success : MessagePack.DecodeResult.TokenMismatch;
    }
}
