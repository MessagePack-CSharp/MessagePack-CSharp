...
//
// MEASURED (Zen 5, ShortRun; V4 narrowing-over-UInt64 -> dedicated gate-5 body):
...
public class TryReadUInt32Benchmark
{
    ...
    public void Setup()
    {
        ...
        {
            MessagePack.MessagePackPrimitives.TryWriteUInt32(buffer.AsSpan(offset), Next(), out var written);
            offset += written;
        }
        ...
        for (int n = 0; n < Count; n++)
        {
            var r0 = MessagePack.MessagePackPrimitives.TryReadUInt32(buffer.AsSpan(o0), out var v0, out var c0);
            var r1 = V3::MessagePack.MessagePackPrimitives.TryReadUInt32(buffer.AsSpan(o1), out var v1, out var c1);
        ...
        if (MessagePack.MessagePackPrimitives.TryReadUInt32(probe, out _, out var tn) != MessagePack.DecodeResult.TokenMismatch || tn != 0)
        {
            throw new InvalidOperationException("verify failed: negative fixint");
        }
        probe[0] = 0xcf; // uint64(42): wide but fits
        probe[8] = 42;
        if (MessagePack.MessagePackPrimitives.TryReadUInt32(probe, out var wv, out var wt) != MessagePack.DecodeResult.Success || wv != 42 || wt != 9)
        {
            throw new InvalidOperationException("verify failed: wide in-range");
        }
        probe[0] = 0xd3; // int64(-1): negative
        probe.Slice(1).Fill(0xff);
        if (MessagePack.MessagePackPrimitives.TryReadUInt32(probe, out _, out var dt) != MessagePack.DecodeResult.TokenMismatch || dt != 0)
        {
        ...
        {
            if (MessagePack.MessagePackPrimitives.TryReadUInt32(probe[..cut], out _, out var tc) != MessagePack.DecodeResult.InsufficientBuffer || tc != 5)
            {
    ...
    public uint V4()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (MessagePack.MessagePackPrimitives.TryReadUInt32(buf.Slice(offset), out var v, out var consumed) == MessagePack.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
    ...
    public uint Mpcs()
    {
        ...
        for (int n = 0; n < Count; n++)
        {
            if (V3::MessagePack.MessagePackPrimitives.TryReadUInt32(buf.Slice(offset), out var v, out var consumed) == V3::MessagePack.MessagePackPrimitives.DecodeResult.Success)
            {
                sum += v;
            }
            offset += consumed;
