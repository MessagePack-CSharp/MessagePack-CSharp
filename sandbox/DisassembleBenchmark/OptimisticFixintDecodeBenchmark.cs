...
public class OptimisticFixintDecodeBenchmark
{
    ...
    public void Setup()
    {
        perValue = new MessagePackSerializerOptions(new MessagePack.MessagePackFormatterResolver(new Int4PerValueFormatterFactory()));
        optimistic = new MessagePackSerializerOptions(new MessagePack.MessagePackFormatterResolver(new Int4OptimisticFormatterFactory()));
        cascade = new MessagePackSerializerOptions(new MessagePack.MessagePackFormatterResolver(new Int4CascadeReadFormatterFactory()));
        hybrid = new MessagePackSerializerOptions(new MessagePack.MessagePackFormatterResolver(new Int4HybridReadFormatterFactory()));
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            payloads[i] = V3::MessagePack.MessagePackSerializer.Serialize(poco);
        }
    ...
    where TReadBuffer : struct, IReadBuffer, allows ref struct
{
    ...
    {
        if (global::MessagePackPrimitives.TryReadInt32HybridDr(buffer.GetUnreadSpan(), out var v, out var consumed) == DecodeResult.Success)
        {
    ...
    where TReadBuffer : struct, IReadBuffer, allows ref struct
{
    ...
    {
        if (global::MessagePackPrimitives.TryReadInt32CascadeDr(buffer.GetUnreadSpan(), out var v, out var consumed) == DecodeResult.Success)
        {
            buffer.Advance(consumed);
            return v;
        }
